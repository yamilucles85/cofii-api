...
module.exports = function (CoffiiUser) {
    CoffiiUser.validatesExclusionOf("username", { in: reservedWords });

    CoffiiUser.afterRemote("findById", function (ctx, user, next) {
    ...
    CoffiiUser.beforeRemote("login", function (ctx, modelInstance, next) {
    ...
    CoffiiUser.beforeRemote("create", function (ctx, user, next) {
    ...
    /* Reset Password */
    /*   CoffiiUser.on("resetPasswordRequest", function (info) {
        ...
        info.accessToken.user(function (err, user) {
          ...
          _client.sendEmailWithTemplate({
            ...
            "TemplateModel": {
              ...
              "action_url": "https://web.coffii.co/CoffiiUser/reset?access_token=" + info.accessToken.id,
    ...
    CoffiiUser.avatar = function (id, req, res, cb) {
        ...
        CoffiiUser.findOne({ where: { id: id } }, function (err, user) {
            if (err) cb(err);
            if (!user) {
                err = new Error(
                    "No instance with id " + id + " found for " + CoffiiUser.modelName
                );
                err.statusCode = 404;
                return cb(err);
            }
    ...
    /*   CoffiiUser.registerDevice = function(data, options, cb) {
    ...
    CoffiiUser.search = function (query, options, cb) {
